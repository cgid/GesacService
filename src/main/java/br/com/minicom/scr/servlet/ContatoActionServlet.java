...
public class ContatoActionServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            Log_contato log_contato = new Log_contato();
            ...
            if (null != telefonenovo) {
                for (int i = 0; i < telefonenovo.length; i++) {
                    ...
                    if (10 <= telefonenovo[i].length()) {
                        ...
                        log_contato.setIdUsuario(Integer.parseInt(String.valueOf(request.getSession().getAttribute("usuarioid"))));
                        ...
                        telefone.setDdd(ddd);
                        int idcontato = equery.select(contato);
                        telefone.setTelefone(t);
                        telefone.setIdContato(idcontato);
                        log_contato.setIdContato(idcontato);
                        telefone.setSituacao(1);
                        log_contato.setOperacao("adicionou contato");
                        System.out.println(log_contato.toString());
                        System.out.println(telefone.toString());
                        equery.insert(log_contato);
